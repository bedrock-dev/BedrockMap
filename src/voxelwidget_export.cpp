...
nlohmann::json appendMeshPrimitive(const std::vector<float>& vertices, const std::vector<GLuint>& indices,
                                   int materialIndex, QByteArray& binaryChunk, nlohmann::json& bufferViews,
    ...
    bufferViews.push_back({{"buffer", 0},
                           ...
                           {"target", kArrayBuffer}});
    bufferViews.push_back({{"buffer", 0},
                           {"byteOffset", static_cast<int>(normalOffset)},
                           {"byteLength", normalData.size()},
                           {"target", kArrayBuffer}});
    bufferViews.push_back({{"buffer", 0},
                           {"byteOffset", static_cast<int>(colorOffset)},
                           {"byteLength", colorData.size()},
                           {"target", kArrayBuffer}});
    bufferViews.push_back({{"buffer", 0},
...
bool VoxelWidget::exportGlb(const QString& filePath, QString* errorMessage) const {
    ...
    if (!transparentVertices.empty() && !transparentIndices.empty()) {
        primitives.push_back(
            appendMeshPrimitive(transparentVertices, transparentIndices, 1, binaryChunk, bufferViews, accessors));
    }
    ...
    const std::uint32_t totalLength =
        static_cast<std::uint32_t>(12 + 8 + jsonChunk.size() + 8 + binaryChunk.size());
